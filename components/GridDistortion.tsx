...
const GridDistortion: React.FC<GridDistortionProps> = ({
  ...
  useEffect(() => {
    ...
    if (imageSrc) {
      ...
      uniforms.uTexture.value = canvasTex;
    }
    ...
    const scheduleGlitch = () => {
      ...
      const jitter = (Math.random() - 0.5) * glitchInterval * 0.5 * 1000;
      glitchTimerId = setTimeout(() => {
        glitchActiveRef.current = true;
        glitchFramesRef.current = Math.floor(Math.random() * 8) + 4;
        scheduleGlitch();
      }, glitchInterval * 1000 + jitter);
    };
    scheduleGlitch();
    ...
    const animate = () => {
      ...
      if (glitchActiveRef.current && glitchFramesRef.current > 0) {
        const numSpikes = Math.floor(size * size * 0.15);
        for (let k = 0; k < numSpikes; k++) {
          const idx = 4 * Math.floor(Math.random() * size * size);
          data[idx] += (Math.random() - 0.5) * glitchStrength * 2;
          data[idx + 1] += (Math.random() - 0.5) * glitchStrength * 2;
        }
      ...
      dataTexture.needsUpdate = true;
      if (liveCanvas && uniforms.uTexture.value) {
  ...
  }, [grid, mouse, strength, relaxation, glitchInterval, glitchStrength, imageSrc, liveCanvas]);
