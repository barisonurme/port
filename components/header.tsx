...
const Header = ({ color = '#ffffff' }: { color?: string }) => {
    ...
    return (
        <div
            ref={rootRef}
            className={`flex justify-center items-center w-full top-0 p-4 lg:p-24 z-5000 absolute ${pathname === '/'
                ? 'pt-4 lg:pt-20'
                : 'pt-3 lg:pt-10'
                }`}
        >
            <div className='border-l border-t flex flex-col w-full h-full max-w-7xl backdrop-blur-3xl bg-black/30 p-3 px-4 lg:p-8 lg:px-12 rounded-[24px] shadow-xl border-white/20'>
                <div className='flex justify-between  items-center w-full h-full gap-2'>
                    ...
                    >
                        <Logo color={color} />
                        <span className='text-sm lg:text-lg'>BARISONURME</span>
                    </button>
                    ...
                    <button
                        ...
                        aria-expanded={open}
                        onClick={() => setOpen((v) => !v)}
                        className='lg:hidden flex flex-col justify-center items-center gap-[5px] w-10 h-10 shrink-0 -mr-1'
                    >
                        <span
                ...
                {/* Mobile menu */}
                <nav
                    className={`lg:hidden flex flex-col overflow-hidden transition-all duration-300 ease-in-out ${open ? 'max-h-60 mt-3 opacity-100' : 'max-h-0 opacity-0'}`}
                >
                    {NAV_LINKS.map((link) => {
